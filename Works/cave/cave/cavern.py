...
def fill(cave, row, col, color):
	...
	if (row+1)<len(cave) and cave[row+1][col]==AIR:
		fill(cave,row+1,col,color)
	if (col+1)<len(cave[0]) and cave[row][col+1]==AIR:
		fill(cave,row,col+1,color)
	if (row-1)>=0 and cave[row-1][col]==AIR:
		fill(cave,row-1,col,color)
	if (col-1)>=0 and cave[row][col-1]==AIR:
		fill(cave,row,col-1,color)
...
def main():
	...
	try:
		...
		print("Error for settings file")
		input()
		exit(1)
	caves = settings.read().splitlines()
	for cav in caves:
		cave = read_cave(cav)
	#	dump_cave(cave)  ## May be useful for debugging
		...
		print(i,"chambers in cavern")
		input("Press enter for next cave")
