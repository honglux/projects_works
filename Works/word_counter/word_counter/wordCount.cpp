...
WordCount::MRmap(const std::map<std::string,std::string> &input,
	...
	for (auto it = input.begin(); it != input.end(); it++ ) {
		...
		do {
			std::string word="";
			iss >> word;
			// go thorght word and delete the punctuations 
			for(int i=0; i<word.length(); i++) {
        		if( word.at(i) == '_' || word.at(i) == ')' || word.at(i) == '(' || word.at(i) == '>' || word.at(i) == ',' || word.at(i) == '<' || word.at(i) == '.' || word.at(i) == ';' || word.at(i) == '!' || word.at(i) == ':'|| word.at(i) == '?') {
        			 word.erase (i,1); i--;
        		}
			}
