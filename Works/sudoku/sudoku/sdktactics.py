...
def naked_single(group):
		...
		"""
		for tile in group:
			if tile.symbol=='.':	#find a unsolved tile
				for ano_til in group:	#another tile that remove choices
					if ano_til.symbol!='.' and len(tile.possible)>1:	#find another tile 
...
def hidden_single(group):
		...
		for tile in group:
			if tile.symbol in digits:	
				digits=digits-set(tile.symbol)	#remove the digits we already have
		for number in digits:
			counter=0
			for tile in group:
				if number in tile.possible:	#find the tile the digits can place
					counter+=1
					temp_tile=tile	#remember the last tile
			if counter==1:	#if we find the only way
