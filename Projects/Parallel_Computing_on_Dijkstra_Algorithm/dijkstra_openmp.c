...
struct Edge dijs_openmp(struct Edge* edges,int edgen,int destna,int print_flag);
...
int main(int argc, char *argv[]) 
{
	...
	result_edge = dijs_openmp(edges,edgen,destna,print_flag);
...
struct Edge dijs_openmp(struct Edge* edges,int edgen,int destna,int print_flag)
{
	...
	while(!checkifinside(cluster,destna,cluster_size))
	{
		...
		for(pi = 0;pi<cluster_size;pi++)
		{
			int pj;
			for(pj = 0;pj<=destna;pj++)
			{
				if(checkconnfsource(edges,edgen,cluster[pi],pj) && !checkifinside(cluster,pj,cluster_size)
